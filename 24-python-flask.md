...
1. **Пакет делается в P2, после первой публикации P1, последним (после Wagtail)** — принято 2026-09-10 (спека 26 §9.5): ценность —
   SEO-имя и `flask indexnow` CLI; всё техническое покрывает core `wsgi` + рецепт, поэтому релиз P1 его не ждёт.
