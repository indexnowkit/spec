...
### 16.5. Аудит 0.13 (2026-09-07) — проведён, ЗАКРЫТ ПОЛНОСТЬЮ той же волной (решение пользователя: «закрываем все до релиза»)

Шестилинзовый аудит семейства на `main` b4a63f5 (после волны Yii3 и hotfix'а §16.4, до пуша и тегов волны):
`docs/plans/audit-0.13.md`, отчёты линз — `docs/plans/audit-0.13/*.md`. Итог: находки аудита 0.10 закрыты без регрессий;
**2 CRITICAL** (R1 — verify-on-commit в yii2/yii3 сравнивает PHP-типы с сырыми строками драйвера и молча выбрасывает
вставки с DECIMAL/timestamptz-колонками; W1 — `Config::fromArray()` читает `dry_run` и три других флага через `(bool)`,
так что `INDEXNOW_DRY_RUN=false` в Yii3 включает dry-run), **18 HIGH** (в основном yii3: `upsert()` не отправляет, хелпер
строится вне `try/catch`, ошибка чтения транзакции отдаёт URL мимо стейджинга; в семье: DSN без установленного history
печатается открытым текстом, узел `clock` не доходит до командных сабмиттеров и не открыт в трёх адаптерах, `forChanges()`
только в yii3, четыре копии `VerifySampleCheck`), ~40 MEDIUM. План — §6 документа аудита: блок 1 (CRITICAL + блокеры yii3 +
аддитивное ядро) предлагается закрыть **до тега** текущей волны без смены версий; блоки 2–4 — волной K, затем Битрикс.
Решения приняты по рекомендациям: всё — до тега; S5 — pre-flight verify всегда на собственном PSR-18-клиенте ядра
(`http.client` приложения только для POST; verify 0.4.0); S6 — `<loc>` фильтруются по `managedHosts()` (sitemap 0.8.0);
R11 — поведение оставлено, строка `router.base_url` в `check` Yii3; A11/A12 — тексты. Уточнение R1 по ходу (yii2/yii3): для
**вставки** «только существование» неверно — откат к savepoint освобождает первичный ключ и sqlite отдаёт его следующей вставке
(конформанс A05c), поэтому вставка и upsert сверяются с записанными колонками, а неоднозначные типы отсеивает
`VerifyingStaging::rowMatches()` ядра; ключ субъекта (`class#pk`) и слияние — только для обновления и удаления. Версии волны
после аудита: core 0.13.0, console 0.4.2, testing 0.3.2, sitemap 0.8.0, verify 0.4.0, history 0.3.1, doctrine 0.9.0,
symfony-bundle 0.15.0, laravel 0.15.0, yii2 0.14.0, yii3 0.1.0 (конструкторы наблюдателей Laravel/Yii2/Doctrine, verify без
